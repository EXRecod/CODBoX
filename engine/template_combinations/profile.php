...
//echo '</br>'.$rp.'</br>';
        ...
        if (is_object($xz))
            foreach ($xz as $keym => $value) {
                ...
                else if ($keym == 's_port') $serverportt = $value;
				...
				else if ($keym == 's_fall') $total_played_time = $value;
...
//abs(get_percentage($total_players_ondatabase - $skillPlace, $total_players_ondatabase));
	...
	//if(empty($total_played_time))
	   //	 $total_played_time = $lasttimeseen;
    // else
	$total_played_time = convertSecToTime($total_played_time);
}
...
//////////////////////////////////////////////////////////////// ajax 
    ...
    echo get_local_source_db($domain . '/engine/template/stats_dynamics.php?guid=' . $guidn, '160000');

    echo '<div style="display: none;visibility: hidden;">';
    if (!empty($_GET['server'])) {
        echo get_local_source_db($domain . '/engine/template/stats_playermaps_frame.php?guid=' . $guid . '&statsmedal=' . $_GET['server'], '160000');
    } else {
        echo get_local_source_db($domain . '/engine/template/stats_playermaps_frame.php?guid=' . $guid, '160000');
    }
    echo '</div>';

    if (!empty($_GET['server']))
        echo get_local_source_db($domain . '/engine/template/stats_playermaps.php?guid=' . $guid . '&statsmedal=' . $_GET['server'], '160000');
    else
        echo get_local_source_db($domain . '/engine/template/stats_playermaps.php?guid=' . $guid . '&statsmedal=' . $serverportt, '160000');
