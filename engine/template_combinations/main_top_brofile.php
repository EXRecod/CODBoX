...
////////////////////
 ...
 ORDER BY (t2.w_skill+0)';
  ...
  $serverssqLdata = 't0.s_port="' . $server . '" and (DATE_SUB(CURDATE(),INTERVAL '.$StatsDaysLimit.' DAY) <= t0.s_lasttime and t1.s_kills >= 100)
 ORDER BY (t2.w_skill+0)';
  $serverFiltr = 'SELECT count(*) as totalpl FROM db_stats_0 where s_port=' . $server;
}
else {
  $serverssqLdata = ' DATE_SUB(CURDATE(),INTERVAL '.$StatsDaysLimit.' DAY) <= t0.s_lasttime and t1.s_kills >= 100
 ORDER BY (t2.w_skill+0)';
  $serverFiltr = 'SELECT count(*) as totalpl FROM db_stats_1 where s_kills >= 100';
}
