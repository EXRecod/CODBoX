...
{ 
if(isLoginUser())
$txt  =  '<b class="rainbowQ">'.$txttitle.'</b> '.$txt;
}
...
<a href="'.$domain.'/chat.php?geo='.$flag.'" class="tags" glose="'.$cn_nm.'">
...
<div style="color:#fff;font-size:13px;display:inline-block;float:left;TEXT-ALIGN:LEFT;">&nbsp <a href="'.$domain.'/chat.php?#" style="color:#fff;" title="'.$txttitle.'">	'.colorize($txt).$txta.'</a></div> 
...
$pageskey = '<a href="'.$domain.'/chat.php?server=' . $server .'&search=' . $search .
'&geo=' . $geosearch .
'&timeq=' . $timeq .
'&page=';
